...
class ModuleInstaller {
    ...
    public static function install() {
        ...
        // We need to find and copy just the .php module file(s)
        $installed = array();
        for ($i = 0; $i < $zip->numFiles; $i++) {
            $name = $zip->getNameIndex($i);
            // Only grab .php files that are directly in the repo root or one level deep
            // and not hidden files or directories
            if (substr($name, -4) === '.php' && substr(basename($name), 0, 1) !== '.') {
                $depth = substr_count(trim($name, '/'), '/');
                if ($depth <= 1) {
                    $filename = basename($name);
                    $contents = $zip->getFromIndex($i);
                    // Verify it has a Module ID header before installing
                    if (preg_match('/Module ID:\s*(.+)/i', $contents, $match)) {
                        file_put_contents($modulesDir . $filename, $contents);
                        $installed[] = $filename;
                    }
                }
            }
